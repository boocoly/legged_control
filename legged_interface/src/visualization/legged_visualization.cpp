...
LeggedVisualizer::LeggedVisualizer(ocs2::PinocchioInterface pinocchioInterface, ocs2::CentroidalModelInfo centroidalModelInfo,
...
void LeggedVisualizer::launchVisualizerNode(ros::NodeHandle &nodeHandle) {
    costDesiredBasePositionPublisher_ = nodeHandle.advertise<visualization_msgs::Marker>("/legged/desiredBaseTrajectory", 1);
    costDesiredFeetPositionPublishers_.resize(centroidalModelInfo_.numThreeDofContacts);
    costDesiredFeetPositionPublishers_[0] = nodeHandle.advertise<visualization_msgs::Marker>("/legged/desiredFeetTrajectory/LF", 1);
    costDesiredFeetPositionPublishers_[1] = nodeHandle.advertise<visualization_msgs::Marker>("/legged/desiredFeetTrajectory/RF", 1);
    costDesiredFeetPositionPublishers_[2] = nodeHandle.advertise<visualization_msgs::Marker>("/legged/desiredFeetTrajectory/LH", 1);
    costDesiredFeetPositionPublishers_[3] = nodeHandle.advertise<visualization_msgs::Marker>("/legged/desiredFeetTrajectory/RH", 1);
    stateOptimizedPublisher_ = nodeHandle.advertise<visualization_msgs::MarkerArray>("/legged/optimizedStateTrajectory", 1);
    currentStatePublisher_ = nodeHandle.advertise<visualization_msgs::MarkerArray>("/legged/currentState", 1);

    // Load URDF model
    urdf::Model urdfModel;
    if (!urdfModel.initParam("legged_robot_description")) {
        std::cerr << "[LeggedVisualizer] Could not read URDF from: \"legged_description\"" << std::endl;
    } else {
        KDL::Tree kdlTree;
...
void LeggedVisualizer::update(const ocs2::SystemObservation &observation, const ocs2::PrimalSolution &primalSolution,
...
void LeggedVisualizer::publishOptimizedStateTrajectory(ros::Time timeStamp, const ocs2::scalar_array_t &mpcTimeTrajectory,
 ...
 */
void LeggedVisualizer::publishDesiredTrajectory(ros::Time timeStamp, const ocs2::TargetTrajectories &targetTrajectories) {
...
void LeggedVisualizer::publishObservation(ros::Time timeStamp, const ocs2::SystemObservation &observation) {
 ...
 */
void LeggedVisualizer::publishCartesianMarkers(ros::Time timeStamp, const ocs2::legged_robot::contact_flag_t &contactFlags,
...
void LeggedVisualizer::publishBaseTransform(ros::Time timeStamp, const ocs2::vector_t &basePose) {
...
void LeggedVisualizer::publishJointTransforms(ros::Time timeStamp, const ocs2::vector_t &jointAngles) const {
    if (robotStatePublisherPtr_ != nullptr) {
        std::map<std::string, scalar_t> jointPositions{{"LF_HAA", jointAngles[0]}, {"LF_HFE", jointAngles[1]},  {"LF_KFE", jointAngles[2]},
                                                       ...
                                                       {"joint1", jointAngles[12]}, {"joint2", jointAngles[13]},
                                                       {"joint3", jointAngles[14]}, {"joint4", jointAngles[15]},
                                                       {"joint5", jointAngles[16]}, {"joint6", jointAngles[17]}
        };
